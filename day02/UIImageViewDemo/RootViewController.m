...
- (void)viewDidLoad
{
    ...
    //通过图片设置背景颜色，但是图片尺寸必须和平面大小刚好合适，否则是平铺填满
    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"back2.jpg"]];
    //UIImageView图片视图，用于显示图片对象的类，继承于UIView
    //UIImage iOS中的图片类，不能直接用于显示，需要借助于UIImageView的对象，UIImageView和UIImage的关系类似于UILabel和NSString
    //初始化图片视图,设置起始位置和大小
    ...
    //UIImage  图片类
    //第一种创建图片对象的方式
    //这种创建方式，图片会被缓存在内存中，不会被释放，占用内存，通常这种方式创建图片用于长期不变，并且大量重复使用的图片，适用于小图片
#if 0
    ...
    //第二种创建方式
    //获取图片的路径
    //NSBundle mainBundle 应用程序的包  在包中查找相应的文件，并返回文件的路径
...
//    frame.origin = CGPointMake(0, screenSize.height-size.height);
    //重设frame
...
-(void)addAnimation
{
    //找到图片视图
    UIImageView * imageView = (id)[self.view viewWithTag:10];
    //所谓动画就是在极短的时间内，在图片视图上切换连续动作的图片对象
    //1.需要一组连续动作的图片
    //初始化可变数组，用于管理动画图片组
