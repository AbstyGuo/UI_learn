...
{
    CustomPageControl * _pageControl;//记录当前显示的图片页码
...
-(void)createScrollView
{
    ...
    //设置显示内容的尺寸
    scrollView.contentSize = CGSizeMake(11 * scrollView.frame.size.width, scrollView.frame.size.height);
}
...
-(void)createPageControl
{
    ...
    _pageControl = [[CustomPageControl alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - 70, self.view.frame.size.width, 50)];
    //设置背景颜色
...
-(void)pageIndexChanged:(UIPageControl *)pageControl
{
    ...
    // 判断是否出现两次一样的currentPage
    if(_oldIndex == pageControl.currentPage)
    {
        if(_oldIndex == 0)
        {
            // 在第一张的位置，应跳转到最后一张图片的位置
    ...
    NSLog(@"第%ld张",pageControl.currentPage);
