...
-(void)createControl
{
     ...
     */
    _activityView.frame = CGRectMake(50, 50, 100, 100);
    _activityView.center = self.view.center;
    
    //改变大小
    _activityView.transform = CGAffineTransformMakeScale(3, 3);
    [self.view addSubview:_activityView];//添加到当前视图上
    [_activityView startAnimating];//启动动画，动画一旦停止，_activityView就消失
    //延迟5秒后停止动画
    [_activityView performSelector:@selector(stopAnimating) withObject:nil afterDelay:5];
    //系统提供的一个加载指示器，属于UIApplication对象，可以设置它可以见或不可见
    [UIApplication sharedApplication].networkActivityIndicatorVisible = YES;
    
}

#pragma mark - 视图将要消失或取消加载时
